"""
This file does the initial processing of the palette colors in order to input
...
def read_palette_colors_file(palettes_filename='LAB_palettes.txt',
    ...
    """
    This function simply reads the file created by the writeRGBColorsToFile
    function
    :param filename:
    :return:
    ...
    pixel_pattern = r'\[-*[0-9]+\.[0-9]+, -*[0-9]+\.[0-9]+, -*[0-9]+\.[0-9]+\]'
    float_pattern = r'-*[0-9]+\.[0-9]+'

    if not os.path.exists(palettes_filepath):
        writeLABColorsToFile(filename=palettes_filename)

    roguesfile = Configs.ProcessingFolderPath + \
        'file_palette_RGBmapping_rogues.txt'
    roguefilelist = [line.strip() for line in open(roguesfile)]

    with open(palettes_filepath) as file:
        for line in file:
            ...
            if name not in roguefilelist:
                ...
                for pixel in pixels:
                    float_pixel = list(map(float, re.findall(r'' +
                                                             float_pattern,
                                                             pixel)))
...
# the artist palettes dict will contain all the palettes from the
# lab_palettes.txt file if this file does not exist then it will be created
# from the palettes and slices folders respectively
...
def getArtistPaletteCount(imagename):
    """
    returns the count of colors in the palette for a single image from
    artistpalettes_dict
    :param imagename:
    :return:
    """
...
def DisplaySIngleImageFromPath(filepath=images_fullsized_dir +
                               'AboardColor610.png'):
    """
    display single image in filepath
    """
